...
public class StagedOutputJob extends Job implements Callable<Boolean> {
  ...
  /**
   * Creates a job which using a temporary staging location for the output data. The data is only
   * copied to the final output directory on successful completion of the job. This prevents
   * existing output data from being overwritten unless the job completes successfully.
   *
   * @param conf
   *          configuration
   * @param jobName
   ...
   *          where to stage output temporarily
   * @param outputPath
   *          output path
   * @param log
   *          logger
  ...
  public static StagedOutputJob createStagedJob(Configuration conf, String jobName, List<String> inputPaths,
    ...
    if (inputPaths != null) {
      try {
        String inputPathStr = StringUtils.join(inputPaths.iterator(), ",");
        FileInputFormat.setInputPaths(retVal, inputPathStr);
      } catch (IOException e) {
  ...
  /**
   * Gets path to store the counters. If this is not set then by default the counters will be stored
  ...
  /**
   * Sets path to store the counters. If this is not set then by default the counters will be stored
   * in the output directory.
   *
   * @param path
   *          parent path for counters
   */
  ...
  public Boolean call() throws Exception {
    try {
      ...
      if (success) {
        _log.info(String.format("Job %s with ID %s succeeded! Tracking URL: %s", getJobName(), jobId,
            this.getTrackingURL()));
      } else {
        _log.error(String.format("Job %s with ID %s failed! Tracking URL: %s", getJobName(), jobId,
            this.getTrackingURL()));
      }
  ...
  public List<Path> getInputPaths() {
    return Arrays.asList(FileInputFormat.getInputPaths(this));
  }

  /**
   * Run the job and wait for it to complete. Output will be temporarily stored under the staging
   * path. If the job is successful it will be moved to the final location.
   */
  @Override
  ...
  private void writeCounters(final FileSystem fs) throws IOException {
    ...
    if (_countersParentPath != null) {
      ...
      // make the name as unique as possible in this case because this may
      // be a directory
    ...
    org.apache.hadoop.mapred.JobID oldJobId =
        new org.apache.hadoop.mapred.JobID(jobID.getJtIdentifier(), jobID.getId());
    ...
    // Collect statistics on successful/failed/killed task attempts,
    // categorized by
    // setup/map/reduce/cleanup.
    // Unfortunately the job client doesn't have an easier way to get these
    // statistics.
    ...
    if (success) {
      ...
      writeAndLog(writer,
          String.format("MAP_REDUCE_SUM_TIME_MS=%d", (long) mapStats.getSum() + (long) reduceStats.getSum()));
   ...
   * @throws IOException
   */
  private List<TaskCompletionEvent> getTaskCompletionEvents(JobClient jobClient, org.apache.hadoop.mapred.JobID jobId)
      throws IOException {
    List<TaskCompletionEvent> events = new ArrayList<TaskCompletionEvent>();

    // Tries to use reflection to get access to the getTaskCompletionEvents
    // method from the private
    // jobSubmitClient field.
    // This method has a parameter for the size, which defaults to 10 for
    // the top level methods and
    // can therefore be extremely slow
    ...
    try {
      ...
      if (jobSubmitClient != null) {
        getTaskCompletionEventsMethod =
            jobSubmitClient.getClass().getDeclaredMethod("getTaskCompletionEvents",
                org.apache.hadoop.mapred.JobID.class, int.class, int.class);
    ...
    while (true) {
      ...
      if (getTaskCompletionEventsMethod != null) {
        try {
          // grab events, 250 at a time, which is faster than the
          // other method which defaults to 10
          // at a time (with no override ability)
          currentEvents =
              (TaskCompletionEvent[]) getTaskCompletionEventsMethod.invoke(jobSubmitClient, jobId, index, 250);
        } catch (IllegalArgumentException e) {
